...
reg  [7:0]  q_nrzi_data, q_nrzi_data2, q_smark,  q_smark2;
wire [7:0]  w_nrzi_data;
wire [10:0] w_smark;
wire [7:0]  w_align_data;
wire [1:0]  w_align_data_pre;
reg [7:0]   w_align_data_q;
reg [1:0]   w_align_data_pre_q;
reg  [2:0]  q_shift;
reg  [2:0]  q_shift_q;
reg  [2:0]  state;
reg  [3:0]  q_offset;
...
reg [18:0] w_data_19_q, w_ofs_mask_q;
wire [7:0]  w_data_raw;
reg  [3:0]  q_data_dly;
wire        w_sync;
reg  [7:0]  q_rxd_0, q_rxd_1, q_rxd_2;
reg         q_rxd_val_0, q_rxd_val_1, q_rxd_val_2;
reg [5:0]   rxd_val_q;
...
always @(posedge clk)
    if(!rstn) begin
         ...
         q_rxd_val_2 <= q_rxd_val_1;
         rxd_val_q[5:0] <= {rxd_val_q[4:0], w_rxd_val};
        if (w_rxd_val) begin
            q_rxd_0 <= w_rxd;
            q_rxd_1 <= q_rxd_0;
            q_rxd_2 <= q_rxd_1;
...
assign w_eop = (((q_offset == 7)&(w_data_19_q[14:8] == 7'b1111_111)) ||
                ((q_offset == 6)&(w_data_19_q[13:7] == 7'b1111_111)) ||
                ((q_offset == 5)&(w_data_19_q[12:6] == 7'b1111_111)) ||
                ((q_offset == 4)&(w_data_19_q[11:5] == 7'b1111_111)) ||
                ((q_offset == 3)&(w_data_19_q[10:4] == 7'b1111_111)) ||
                ((q_offset == 2)&(w_data_19_q[09:3] == 7'b1111_111)) ||
                ((q_offset == 1)&(w_data_19_q[08:2] == 7'b1111_111)) ||
                ((q_offset == 0)&(w_data_19_q[07:1] == 7'b1111_111)));
...
assign w_align_data_pre = (q_shift==3'h1) ? {q_rxd_0[0],q_rxd_1[7]} :
                          (q_shift==3'h2) ? {q_rxd_0[1:0]} :
                          (q_shift==3'h3) ? {q_rxd_0[2:1]} :
                          (q_shift==3'h4) ? {q_rxd_0[3:2]} :
                          (q_shift==3'h5) ? {q_rxd_0[4:3]} :
                          (q_shift==3'h6) ? {q_rxd_0[5:4]} :
                          (q_shift==3'h7) ? {q_rxd_0[6:5]} : q_rxd_1[7:6];
...
always @(posedge clk) begin
    if (!rstn) begin
        w_align_data_q <= 'd0;
        w_align_data_pre_q <= 'd0;
    end else begin
        w_align_data_q <= w_align_data;
        w_align_data_pre_q <= w_align_data_pre;
    end
end
...
reg [2:0] w_sync_q;
reg [1:0] sync_bitstuff_q;
wire [6:0] sync_bitstuff_wires;
wire sync_bitstuff;
assign sync_bitstuff_wires = {w_align_data_q[4:0], w_align_data_pre_q[1:0]};
assign sync_bitstuff =  w_sync_q[1] && (&(sync_bitstuff_wires) || 
                        ~|(sync_bitstuff_wires));
always @(posedge clk) begin
    if (!rstn) begin
        w_sync_q[2:0] <= 'd0;
        sync_bitstuff_q[1:0] <= 'd0;
    end else begin
        w_sync_q[2:0] <= {w_sync_q[0], w_sync};
        sync_bitstuff_q[1:0] <= {sync_bitstuff_q[0], sync_bitstuff};
    end
end

////nrzi removal////////////
assign w_nrzi_data = {!(w_align_data_q[7]^w_align_data_q[6]),
                      !(w_align_data_q[6]^w_align_data_q[5]),
                      !(w_align_data_q[5]^w_align_data_q[4]),
                      !(w_align_data_q[4]^w_align_data_q[3]),
                      !(w_align_data_q[3]^w_align_data_q[2]),
                      !(w_align_data_q[2]^w_align_data_q[1]),
                      !(w_align_data_q[1]^w_align_data_q[0]),
                      !(w_align_data_q[0]^w_align_data_pre_q[1])};
...
always @(posedge clk) begin
    if(!rstn) begin
        ...
        if (rxd_val_q[0]) begin //rxd_val_fixed
...
always @(posedge clk) begin
    if(!rstn) begin
        ...
        w_data_19_q  <= w_data_19;
        w_ofs_mask_q <= w_ofs_mask;
    end
end

assign w_smark_19 = {w_smark, q_smark} & w_ofs_mask_q;
assign w_smark = sync_bitstuff_q[1] ? 11'b00000100000 :  
                (state==IDLE_STATE) ? 'd0 : { &{~w_data_19_q[18],w_data_19_q[17:12]} //! 'd0 - беда с разрядностью?
                                              ,&{~w_data_19_q[17],w_data_19_q[16:11]}
                                              ,&{~w_data_19_q[16],w_data_19_q[15:10]}
                                              ,&{~w_data_19_q[15],w_data_19_q[14:9]}
                                              ,&{~w_data_19_q[14],w_data_19_q[13:8]}
                                              ,&{~w_data_19_q[13],w_data_19_q[12:7]}
                                              ,&{~w_data_19_q[12],w_data_19_q[11:6]}
                                              ,&{~w_data_19_q[11],w_data_19_q[10:5]}
                                              ,&{~w_data_19_q[10],w_data_19_q[ 9:4]}
                                              ,&{~w_data_19_q[ 9],w_data_19_q[ 8:3]}
                                              ,&{~w_data_19_q[ 8],w_data_19_q[ 7:2]}}; //! почему не проверяем дальше????????

wire [9:0] w_ofs_smark;
reg [9:0] w_ofs_smark_q;
wire [3:0] w_offset_inc;
reg [3:0] w_offset_inc_q;
...
always @(posedge clk) begin
    if(!rstn) begin
        ...
        if (state == DATA_STATE) begin
            if (rxd_val_q[2]) begin //rxd_val_fixed ПОМЕНЯТЬ НА 2!!!!!!!!!!!!!
        ...
        q_offset <= (state==DATA_STATE) ? (rxd_val_q[2] ? (q_offset>=4'h8 ? q_offset-4'h8:q_offset+w_offset_inc) : q_offset) : 0;
        q_smark <= w_smark[7:0];
        q_smark2 <= q_smark;
        q_data_dly[3:0] <= {q_data_dly[2:0], state==DATA_STATE};
        w_offset_inc_q <= w_offset_inc;
    end
end
...
assign w_data_raw = (w_ofs_smark==10'b0010000001) ? {w_data_19_q[q_offset+8+:2], w_data_19_q[q_offset+1+:6]}:
                    (w_ofs_smark==10'b0100000010) ? {w_data_19_q[q_offset+9], w_data_19_q[q_offset+2+:6], w_data_19_q[q_offset]}:
                    (w_ofs_smark==10'b1000000100) ? {w_data_19_q[q_offset+3+:6], w_data_19_q[q_offset+:2]}:
                    (w_ofs_smark==10'b0100000001) ? {w_data_19_q[q_offset+9], w_data_19_q[q_offset+1+:7]}:
                    (w_ofs_smark==10'b1000000010) ? {w_data_19_q[q_offset+2+:7], w_data_19_q[q_offset]}:
                    (w_ofs_smark==10'b1000000001) ? w_data_19_q[q_offset+1+:8]:
                    (w_ofs_smark==10'b0000000001) ? w_data_19_q[q_offset+1+:8]:
                    (w_ofs_smark==10'b0000000010) ? {w_data_19_q[q_offset+2+:7], w_data_19_q[q_offset]}:
                    (w_ofs_smark==10'b0000000100) ? {w_data_19_q[q_offset+3+:6], w_data_19_q[q_offset+:2]}:
                    (w_ofs_smark==10'b0000001000) ? {w_data_19_q[q_offset+4+:5], w_data_19_q[q_offset+:3]}:
                    (w_ofs_smark==10'b0000010000) ? {w_data_19_q[q_offset+5+:4], w_data_19_q[q_offset+:4]}:
                    (w_ofs_smark==10'b0000100000) ? {w_data_19_q[q_offset+6+:3], w_data_19_q[q_offset+:5]}:
                    (w_ofs_smark==10'b0001000000) ? {w_data_19_q[q_offset+7+:2], w_data_19_q[q_offset+:6]}:
                    (w_ofs_smark==10'b0010000000) ? {w_data_19_q[q_offset+8], w_data_19_q[q_offset+:7]}: w_data_19_q[q_offset+:8];


assign DataIn_o = w_data_raw;
assign RxActive_o = (state != IDLE_STATE) & (state!=EOP_STATE) & (!w_eop);
assign RxValid_o = RxActive_o & ((q_shift == 0) ? q_data_dly[2] : q_data_dly[1]) & (q_offset<8) & rxd_val_q[2];
...
always @(posedge clk)
  if(!rstn)
    ...
    case (state)
      IDLE_STATE:
        if(w_sync_q[1])              //data bus exits se0 idle state
          state <= DATA_STATE;
      DATA_STATE:
        if (w_eop)
          state <= EOP_STATE;
