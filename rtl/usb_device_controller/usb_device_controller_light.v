...
(
    ...
    //#lightened
    //Полностью вырезал test_packet и все его сигналы
    //Test Packet
    //
    //#lightened 
    wire       usbp_txvalid_o;
    ...
    wire [9:0] dev_packet_dbg_o;
    wire [9:0] dev_control_dbg_o;
    wire [9:0] dev_transact_dbg_o;

    assign dev_controller_dbg_o[0]  = isync[1];
    assign dev_controller_dbg_o[1]  = osync[1];
    assign dev_controller_dbg_o[2]  = usbt_fin;
    assign dev_controller_dbg_o[3]  = usbc_clr_in[1];
    assign dev_controller_dbg_o[4]  = usbc_clr_out[1];
    assign dev_controller_dbg_o[5]  = dev_control_dbg_o[0];
    assign dev_controller_dbg_o[6]  = dev_control_dbg_o[1];
    assign dev_controller_dbg_o[7]  = 1'b0;
    assign dev_controller_dbg_o[8]  = 1'b0;
    assign dev_controller_dbg_o[9]  = 1'b0;
    assign dev_controller_dbg_o[10] = 1'b0;
    assign dev_controller_dbg_o[11] = 1'b0;
    assign dev_controller_dbg_o[12] = 1'b0;
    assign dev_controller_dbg_o[13] = 1'b0;
    assign dev_controller_dbg_o[14] = 1'b0;
    assign dev_controller_dbg_o[15] = 1'b0;
    assign dev_controller_dbg_o[16] = 1'b0;
    assign dev_controller_dbg_o[17] = 1'b0;
    assign dev_controller_dbg_o[18] = 1'b0;
    assign dev_controller_dbg_o[19] = 1'b0;
    ...
    usb_packet u_usb_packet(
        ...
        ,.dev_packet_dbg_o(dev_packet_dbg_o)
    );
    ...
    usb_transact usb_transact_inst(
         .CLK             (clk_i         )
        ,.RESET           (usbi_usbrst   )
        ,.T_IN            (usbt_in       )
        ,.T_OUT           (usbt_out      )
        ...
        ,.dev_transact_dbg_o (dev_transact_dbg_o)
    );
    ...
    usb_control_inst (
        ...
        ,.T_TXRDY                (usbt_txrdy      )
        ,.T_TXDAT                (usbc_txdat      )
        ,.dev_control_dbg_o      (dev_control_dbg_o)
    );
    ...
    always @(posedge clk_i) begin
        if (s_reset == 1) begin
            ...
            //s_txbuf_stop <= (txdat_len_i > MAX_HSPACKET_SIZE ? MAX_HSPACKET_SIZE : txdat_len_i); //heavy
            s_txbuf_stop <= txdat_len_i; //#lightened, мы не должны ставить txdat больше 1024!!!
        end
    end
    always @(posedge clk_i) begin
        if (s_reset == 1) begin
...
//======Descriptor ROM Read
    ...
    always @(*) //УБРАТЬ ЭТУ КОМБИНАТОРИКУ
    begin
        descrom_start <= 0;
        usbc_dsclen <= 0;
        case (usbc_dsctyp)
            ...
            8'h06 : begin   // device qualifier
                    descrom_start <= desc_qual_addr_i;
                    usbc_dsclen   <= desc_qual_len_i;
            end

            8'h07 : begin   // other speed configuration
                //#lightened
                if (usbc_dscinx == 0) begin
                    descrom_start <= desc_hscfg_addr_i;
                    usbc_dsclen   <= desc_hscfg_len_i;
...
///////////
    ...
    assign PHY_XCVRSELECT    = s_xcvrselect;
...
(
        ...
        ,output wire [3:0] transact_state_o
        ,output wire stuck_in_2nd_or_3rd_state_o
        ,output wire [9:0] dev_transact_dbg_o
    );
...
module usb_packet (
    ...
    ,output wire [9 : 0] dev_packet_dbg_o
);
...
module usb_control #(
    ...
    ,output reg [9 : 0] dev_control_dbg_o
);
    ...
    assign dev_control_dbg_o[1] = T_STALL;
